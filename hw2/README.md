...
Интеграционные тесты представлены в виде [Postman коллекции](<MAX Disk 360 - Интеграционные тесты.postman_collection.json>) - они покрывают некоторые основные методы системы, проверяют аутентификацию и авторизацию.
...
Для запуска тестов, достаточно запустить [скрипт](run_unit_tests.sh) - данный скрипт поднимет Docker контейнеры с тестами каждого сервиса.